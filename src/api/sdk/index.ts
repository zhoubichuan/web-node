...
// 打开应用
ipcMain.on('openApp', async (event, path) => {
...
// 打开日历
ipcMain.on('calendar', async (event, value) => {
...
// 执行可执行文件
ipcMain.on('execFile', async (event, data) => {
  const {name} = JSON.parse(data)
...
// 流式处理子进程
ipcMain.on('spawn', async (event, data) => {
  const {name} = JSON.parse(data)
...
// 创建 Node.js 子进程
ipcMain.on('fork', async (event, data) => {
  const {name} = JSON.parse(data)
